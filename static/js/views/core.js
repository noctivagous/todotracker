...
/**
 * Left panel minimize/restore toggle (shared between Todos + Notes).
 *
 * The toggle control is rendered in the left browser panel with id `ttLeftPanelToggleBtn`.
 * This binder:
 * - syncs the button UI to the current `data-tt-minimized` state
 * - wires click to toggle the master panel rail/minimized state
 */
function ttBindLeftPanelMinimizeToggle() {
    ...
    // Always sync UI on render (important when switching routes while minimized).
    syncBtn();

    if (leftToggle._ttBound) return;
